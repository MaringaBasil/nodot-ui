...
const FAQS = [
  { q: 'How do I accept a pickup?',         a: 'Go to the Jobs tab, browse available pickups nearby and tap "Accept". You can only have one active job at a time.' },
  { q: 'How is my payout calculated?',      a: 'Payout is based on the weight and material type of your collection. The rate per kg is shown on each job before you accept.' },
  { q: 'When will I be paid?',              a: 'Payouts are processed within 24 hours of completing a pickup. Funds appear in your wallet and can be withdrawn immediately.' },
  { q: 'What if the address is wrong?',     a: 'Cancel the job in the Jobs tab and contact support. Never collect from an unverified or inaccessible address.' },
  { q: 'How do I dispute a weight?',        a: 'Open your History and tap the relevant pickup. Select "Dispute weight" and describe the issue. Our team reviews within 48 hours.' },
];

const CATEGORIES = [
  { label: 'Payments & Wallet', icon: 'wallet-outline' as const,       color: '#2C6E91' },
  { label: 'Job Issues',        icon: 'cube-outline' as const,          color: '#E28F3C' },
  { label: 'Account & ID',      icon: 'person-outline' as const,        color: '#7B52AB' },
  { label: 'App Help',          icon: 'help-circle-outline' as const,   color: '#3F8B7B' },
];
...
function createStyles(C: ReturnType<typeof useTheme>['colors'], isDark: boolean) {
  ...
  return StyleSheet.create({
    ...
    catItem: {
      ...
      backgroundColor: C.card, borderWidth: 1, borderColor: cardBorder, alignItems: 'flex-start',
    },
...
export default function PickerSupport() {
          ...
          >
            ...
            <View style={styles.section}>
              ...
              <View style={styles.contactCard}>
                {[
                  { icon: 'person-outline' as const,  label: 'Picker Support Team', meta: 'Mon–Fri, 07:00–18:00' },
                  { icon: 'mail-outline' as const,     label: 'Email support',       meta: 'pickers@nodot.co.za'  },
                  { icon: 'call-outline' as const,     label: 'Phone',               meta: '+27 11 000 5678'      },
                ].map((c, idx, arr) => (
                  <View key={c.label}>
